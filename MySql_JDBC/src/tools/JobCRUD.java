...
public class JobCRUD implements CRUD {
    ...
    public void show() {
       ...
       for (Job job : jdao.getAll()) {
           ...
           System.out.print(job.getMinSalary() + " -- ");
    ...
    public void insert() {
       ...
       List<String> jobsInsertJidJt = new ArrayList<>();
       for (Job job : jdao.getAll()){
           jobsInsertJidJt.add(job.getJobId());
           jobsInsertJidJt.add(job.getJobTitle());
       }
       

       boolean validasiInsert_JID = jobsInsertJidJt.stream().anyMatch(jobId_Insert::contains);
       boolean validasiInsert_JT = jobsInsertJidJt.stream().anyMatch(jobTitle_Insert::contains);
    ...
    @Override
    public void update() {
       List<String> jobsUpdate = new ArrayList<>();
       for (Job job : jdao.getAll()){
           jobsUpdate.add(job.getJobId());
           jobsUpdate.add(job.getJobTitle());
       }
               
       System.out.print("Job ID: ");
       String jobId_beforeUpdate = scan.nextLine();
       boolean validasiAwalJobID = jobsUpdate.stream().anyMatch(jobId_beforeUpdate::contains);
    ...
    public void delete() {
       ...
       List<String> jobsDeleteID = new ArrayList<>();
       for (Job job : jdao.getAll()){
           jobsDeleteID.add(job.getJobTitle());
       }

       boolean validasiDelete = jobsDeleteID.stream().anyMatch(jobTitle_Delete::contains);
